...
class CalculatorButtonView: UIButton {
    ...
    private var lowlightConstant: CGFloat = 0.1
    private var displayLabel = UILabel()
    
    func loadButtons(view: UIView, display: UILabel) {
    ...
    func createButtonViews() {
        ...
        for (index, name) in enumerate(buttonNames) {
            ...
            button.addTarget(self, action: "highlight:", forControlEvents: UIControlEvents.TouchDown)
            button.addTarget(self, action: "unHighlight:", forControlEvents: UIControlEvents.TouchUpInside)
    ...
    func addButtonConstraints() {
        ...
        for (index, button) in enumerate(self.buttonArray) {
            self.buttonArray[index].snp_makeConstraints { make in
                // Set aspect ratio for a pleasant resize
    ...
    */
    func highlight(sender: UIButton) {
        ...
        sender.backgroundColor?.getRed(&rgb[0], green: &rgb[1], blue: &rgb[2], alpha: &alpha)
        rgb = rgb.map { $0 - self.lowlightConstant }
        sender.backgroundColor? = UIColor(red: rgb[0], green: rgb[1], blue: rgb[2], alpha: alpha)
    }
    /**
